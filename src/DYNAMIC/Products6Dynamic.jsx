import { FaRegHeart } from "react-icons/fa";
import { FiShoppingBag } from "react-icons/fi";
import { IoEyeOutline } from "react-icons/io5";

export default function Products6Dynamic({products6}) {
  return (
    
        <div className=" parentt2 w-auto container px-4 py-10 flex flex-col justify-center items-center ">
          <div className=" relative flex justify-center items-center  ">
            <img src={products6.thumbnail_img} alt={products6.name} className=" w-full lg:h-[15rem]" />
          </div> 
          <div className="">
            <h4 className=" font-bold text-xl flex justify-center items-center lg:text-sm text-center py-4">
              {products6.name}
            </h4>
            <h4 className=" text-orange-500 font-bold flex justify-center items-center">
            ${products6.unit_price} 
            {/* <del className=" text-black">{products6.unit_price}</del> */}
            </h4>
            <div className=" title2 absolute opacity-0  gap-1 text-5xl  flex justify-center items-center">
              <FaRegHeart className=" bg-white p-3 hover:bg-orange-500 hover:text-white duration-500 " />
              <FiShoppingBag className=" bg-white p-3 hover:bg-orange-500 hover:text-white duration-500" />
              <IoEyeOutline className=" bg-white p-3 hover:bg-orange-500 hover:text-white duration-500" />
            </div>
          </div>
        </div>
      );
    }

